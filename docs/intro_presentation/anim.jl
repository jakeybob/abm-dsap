...
β = 0.05 * 1.5 # prob of a contact causing infection
c = 10.0 * δt * 1 # poissonian expectation value of number of contacts in time step
...
i = 1
for num_agents in round.(Int64, range(agents_min, agents_max, n_frames))
    N = num_agents
    abm_model = init_model(β, c, γ, num_agents, I0)
    ...
    plot_title = "β = .05, c = 1, N = " * string(num_agents)
    p = plot(t, abm_data[:, 2], label="S", xlab="time", ylabel="number", title = plot_title, lw=2)
    p = plot!(t,abm_data[:, 3], label="I", lw = 2)
    p = plot!(t,abm_data[:, 4], label="R", lw = 2)

    # path = "pics/" * string(num_agents) * ".png"
    path = "presentation/pics/anim1_$(@sprintf("%d", i)).png"
    savefig(path)
    i = i + 1
end

# ffmpeg -f image2 -framerate 60 -i anim1_%d.png -vcodec libx264 -crf 20 -pix_fmt yuv420p ../anim1.mp4
